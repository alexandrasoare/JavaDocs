...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter((String str) -> str == name).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
        collect = personList.stream().map((Person p) -> p.getName()).collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap((List<Integer> list) -> list.stream()).collect(toList());
    ...
    public static int product(){
        ...
        int product = 1;
        //TODO #8
        product = input.reduce(1, (Integer prod, Integer n) -> prod*n);
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        //TODO #9
        personOptional = personList.stream().filter((Person p) -> p.isMale() == true).findFirst();
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch((Person p) -> p.getAge() < age);
    ...
    public static int findTheMinimumNumber(){
        ...
        int min =0;
        min = input.reduce(Integer.MAX_VALUE, (Integer acc, Integer n) -> Integer.min(acc, n));
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
       //min = personList.stream().map((Person px) -> px.getAge()).min(Integer::compare).get();
        min = personList.stream().map((Person px) -> px.getAge())
                .reduce(Integer.MAX_VALUE, (Integer acc, Integer n) -> Integer.min(acc, n));
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum = personList.stream().map((Person p) -> p.getAge())
                .reduce(0, (Integer s, Integer n) -> s + n);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter((Person p) -> p.getName().startsWith("J"))
                .map((Person p) -> p.getNationality()).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().map((Integer i) -> i*i)
                .filter((Integer i) -> i % 3 == 0).findFirst();
    ...
    public static int sumStringLength(){
        ...
        int sum = 0;
        //TODO #16
        sum = strings.stream().map((String s) -> s.length()).reduce(0, (Integer suma, Integer n) -> suma + n);
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().map((Person p) -> p.getSalary()).reduce(0, (Integer suma, Integer n) -> suma + n);
